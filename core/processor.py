...
class ConnectModProcess(Process):
    ...
    def __get_data(self, area):
        ...
        try:
            if area["func"] == 3:
                self.bytearray_data = self.master.execute(area["slave_id"], cst.READ_HOLDING_REGISTERS, area['start_reg_adr'], area['size'])
                return True
            elif area["func"] == 4:
                self.bytearray_data = self.master.execute(area["slave_id"], cst.READ_INPUT_REGISTERS, area['start_reg_adr'], area['size'])
                return True
            else:
                return False
    ...
    def __thread_for_write_data(self, value_param):
        ...
        if value_param['if_change'] and value_param["table_name"] in self.values and "time_" + value_param["table_name"] in self.values:
    ...
    def __write_to_db(self, tablename, value):
        ...
        self._c.execute(
            f'''INSERT INTO mvlab_{tablename} (value) VALUES ({value});''')
